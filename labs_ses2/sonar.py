...
class Sonar:
    ...
    def __calculer_moyenne_mobile(self, nouv_valeur): #prend une nouvelle valeur et vient calculer la moyenne mobile
        self.__valeurs_passees.append(nouv_valeur) #ajoute la nouvelle distance a la fin du tableau
        
        if len(self.__valeurs_passees)>self.FENETRE: #si la longueur est plus grande que notre fenêtre
            del self.__valeurs_passees[0] #supprimer la première valeur
            
        if len(self.__valeurs_passees) > 2: #si la longueur du tableau est plus grande que 2
            MIN = min(self.__valeurs_passees)
            MAX = max(self.__valeurs_passees)
            moyenne_mobile = (sum(self.__valeurs_passees) - (MIN + MAX))/(len(self.__valeurs_passees)-2) #on calcul la moyenne avec le minimum et le maximum pris en compte
        else: #s'il est plus petit ou égal à 2
            moyenne_mobile = sum(self.__valeurs_passees)/len(self.__valeurs_passees) #on calcul la moyenne sans le minimum et le maximum
    ...
    def when_deactivated(self): #quand le echo a finit
        ...
        self.__distance = self.__calculer_moyenne_mobile(distance_actuelle) #on attribut une distance moyenne
