...
while maycontinue:  # tant qu'on peut continuer
    ...
    if distance_d > DIST_MIN:
        ...
        robot.clignoter_del_verte(
            distance_d
        )  # on fait clignoter la del verte. La vitesse de clignotement (sleep) est controlée par la distance (+ petite = + vite)
    ...
    cv2.putText(
        img, text_dist_g, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (200, 150, 255), 2
    )
    cv2.putText(
        img, text_dist_d, (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (200, 150, 255), 2
    )
